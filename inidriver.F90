    ...
    !     Modified for interacting DM, Francis-Yan Cyr-Racine, Nov 2015
    ...
    !FYCR-DM-DR
    !Set the root for the "dark" output
    P%outroot_dark=outroot
    ...
    !FYCR-DM-DR
    !Read off the dark radiation physical energy density today
    P%omDRh2 = Ini_Read_Double('omDRh2',1.0d-6)
    
    !Read off the dark radiation temperature
    P%xi_dark = Ini_Read_Double('dark_to_visible_temp_ratio',0.5_dl)
    P%t_dark   = P%tcmb*P%xi_dark
    
    !Read off the dark matter mass
    P%m_DM_GeV    = Ini_Read_Double('mass_DM_GeV',10.0_dl)

    !Read in the maximum value of n for the opacity coefficients
    P%DM_DR_highest_n = Ini_Read_Int('DM_DR_highest_n',10)
    !Read in the a_n, b_n, and d_n coefficients for the opacities and heating rate
    numstr = Ini_Read_String('a_DM_DR')
    read(numstr,*) P%a_DM_DR(1:P%DM_DR_highest_n + 1)
    numstr = Ini_Read_String('b_DR_DR')
    read(numstr,*) P%b_DR_DR(1:P%DM_DR_highest_n + 1)
    numstr = Ini_Read_String('d_DM_DR')
    read(numstr,*) P%d_DM_DR(1:P%DM_DR_highest_n + 1)

    !Read in the maximum value of l that is provided for the alpha_l and beta_l coefficients
    P%DM_DR_highest_l = Ini_Read_Int('DM_DR_highest_l',10)
    numstr = Ini_Read_String('alpha_DM_DR_l')
    read(numstr,*) P%alpha_DM_DR_l(2:P%DM_DR_highest_l)
    numstr = Ini_Read_String('beta_DR_DR_l')
    read(numstr,*) P%beta_DR_DR_l(2:P%DM_DR_highest_l)

    !Reference redshift for opacity coefficients
    P%z_D_norm = Ini_Read_Double('z_D',1.0d7)

    !Fraction of the total DM that is in SIDM
    P%f_sidm = Ini_Read_Double('f_sidm',0.99999999_dl)
    
    !Whether to adjust the helium density to take into account the DR
    P%BBN_consistency = Ini_Read_Logical('BBN_consistency',.true.)
    ...
    if (FeedbackLevel> 0) then                 
       print *,'MODIFIED FOR INTERACTING DARK MATTER'
       print *,'Only Available for Scalar Perturbations (so far)'  
    end if
