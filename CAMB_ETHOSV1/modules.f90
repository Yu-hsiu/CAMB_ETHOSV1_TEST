  ...
  module ModelParams
    ...
    !FYCR-DM-DR
    character(LEN=*), parameter :: version = 'Apr14 + Interacting DM'
    ...
    !FYCR-DM-DR
    integer, parameter :: DM_DR_max_n = 10 !The maximum n used in the expansion of the opacities and heating rate
    integer, parameter :: DM_DR_max_l = 10 !The maximum l used in the alpha_l expansion describing the angular dependence
                                           !of the DM-DR cross section
    ...
    !FYCR-DM-DR
    !Whether to adjust the helium density as a function of the DR temperature
    logical  :: BBN_consistency
    !Dark radiation physical energy density today
    real(dl) :: omDRh2
    !Dark radiation temperature and other dark parameters
    real(dl) :: xi_dark !DR to CMB temperature ratio
    real(dl) :: t_dark  !DR temperature
    real(dl) :: m_DM_GeV !DM mass in GeV
    real(dl) :: m_DM !DM mass in SI units
    !Opacity and Heating rate coefficients
    real(dl) :: z_D_norm
    integer  :: DM_DR_highest_n
    real(dl) :: a_DM_DR(DM_DR_max_n)
    real(dl) :: b_DR_DR(DM_DR_max_n)
    real(dl) :: d_DM_DR(DM_DR_max_n)
    !alpha_l coefficients encoding the angular dependence of the cross section
    integer  :: DM_DR_highest_l
    real(dl) :: alpha_DM_DR_l(DM_DR_max_l) !Note that this array contains the alpha_l starting from l=1
    real(dl) :: beta_DR_DR_l(DM_DR_max_l) !Note that this array contains the beta_l starting from l=1
    
    character(LEN=1024) :: outroot_dark
    
    !Fraction of the total DM that is in dark atoms
    real(dl) :: f_sidm
    ...
    !FYCR-DM-DR
    !Energy density in DR and Normal CDM (grhoc is used for SIDM)
    real(dl) grhog_dark,grhoc_cdm
  ...
  subroutine CAMBParams_Set(P, error, DoReion)
    ...
    !FYCR-DM-DR
    !Compute the DM mass in SI units
    CP%m_DM = CP%m_DM_GeV*1.0d9*eV/c**2.d0
    
    !Adjust helium according to DR temperature and degeneracy factor
    if (CP%BBN_consistency) CP%Yhe = GetYhe(CP%omegab*(CP%H0/100)**2,2.0_dl*CP%omDRh2/2.47236d-5)
    ...
    !FYCR-DM-DR
    !DR energy density
    omega_g_h2 = 2.47236d-5*(CP%tcmb/2.7255)**4
    grhog_dark =grhog*(CP%omDRh2/omega_g_h2)
    !grhog_dark = CP%DR_stat_fact*(CP%DR_degeneracy/2.0_dl)*kappa/c**2*4*sigma_boltz/c**3*CP%t_dark**4*Mpc**2
    ...
    !FYCR-DM-DR
    !fraction in SIDM and plain CDM
    grhoc=grhom*CP%omegac*CP%f_sidm
    grhoc_cdm = grhom*CP%omegac*(1.0_dl - CP%f_sidm)
    ...
    !FYCR-DM-DR
    ! Add contribution from DR
    adotrad = sqrt((grhog+grhog_dark+grhornomass+sum(grhormass(1:CP%Nu_mass_eigenstates)))/3)
    ...
    !FYCR-DM-DR
    !Number density of dark matter now
    !FYCR-REMOVE?
    !Nnow_dark = CP%f_sidm*CP%omegac*grhom*c**2/kappa/m_DM/Mpc**2
    ...
    if (present(error)) then
        ...
        !FYCR-DM-DR
        write(*,'("Om_SIDM h^2          = ",f9.6)') CP%omegac*CP%f_sidm*(CP%H0/100)**2
        write(*,'("Om_CDM h^2           = ",f9.6)') CP%omegac*(1.0d0-CP%f_sidm)*(CP%H0/100)**2
        ...
        !FYCR-DM-DR
        write(*,'("Helium Fraction      = ",f9.6)') CP%Yhe
        if (CP%Num_Nu_Massive > 0) then
    ...
    !FYCR-DM-DR
    !Functions to adjust Helium abundance
    !degen is the DR physical energy density today in units
    !of one polarization of the photon density today
    function GetYhe(omegabh2,degen)
      implicit none
      real(dl) GetYhe
      real(dl) omegabh2
      real(dl) degen,S
      S = sqrt(1.0d0 + (4.0d0/43.0d0)*degen)
      GetYhe = 0.2485d0 + 0.0016d0*((273.9d0*omegabh2-6.0d0)+100d0*(S-1.0d0))
    end function GetYhe
...
500     if (L< lmax_extrap_highl) &
    ...
    public
    !FYCR-DM-DR
    !Note the ordering of the transfer function!
    integer, parameter :: Transfer_kh =1, Transfer_sidm=2,Transfer_b=3,Transfer_g=4, &
    Transfer_r=5, Transfer_nu = 6,  & !massless and massive neutrino
    Transfer_tot=7,Transfer_g_dark=8,Transfer_cdm=9

    !FYCR-DM-DR
    integer, parameter :: Transfer_max = Transfer_cdm
        ...
        if (FileNames(i_PK) /= '') then
            ...
            do ik=1,MTrans%num_q_trans
                if (MTrans%TransferData(Transfer_kh,ik,i)/=0) then
                   !FYCR-DM-DR
                   !Added the two extra component for DR and plain CDM
                   write(fileio_unit,'(9E14.6)') MTrans%TransferData(Transfer_kh:Transfer_max,ik,i)
                end if
            end do
            close(fileio_unit)
        end if
    ...
    !ccccccccccccccccccccccccccccccccccccccccccccccccccc
    !FYCR-DM-DR
    !Compute the scattering rate between DM and DR as a function of tau
    module ThermoData_dark
      ...
      real(dl) tb(nthermo_dark),tbold(nthermo_dark),cs2(nthermo_dark)
      real(dl) dcs2(nthermo_dark)
      real(dl) dotmu(nthermo_dark), ddotmu(nthermo_dark)
      real(dl) dddotmu(nthermo_dark),ddddotmu(nthermo_dark)
      real(dl) dotmu_chi(nthermo_dark), ddotmu_chi(nthermo_dark)
      real(dl) dddotmu_chi(nthermo_dark),ddddotmu_chi(nthermo_dark)
      real(dl) dotmu_DR(nthermo_dark), ddotmu_DR(nthermo_dark)
      real(dl) dddotmu_DR(nthermo_dark),ddddotmu_DR(nthermo_dark)
      real(dl) tauminn,dlntau,Maxtau
      real(dl) :: tight_tau_dark, tau_dec_dark,drag_tau_dark,end_drag_dark
      
      public thermo_dark,inithermo_dark,Thermo_OpacityToTime_dark, tight_tau_dark, tau_dec_dark, &
          drag_tau_dark
    contains
      
      subroutine thermo_dark(tau,cs2b,opacity,opacity_chi,opacity_DR, dopacity,dopacity_chi,dopacity_DR)
        ...
        real(dl), intent(out) :: cs2b,opacity,opacity_chi,opacity_DR
        real(dl), intent(out), optional :: dopacity, dopacity_chi,dopacity_DR
        ...
        if (i < 1) then
           ...
           opacity_chi=dotmu_chi(1)+(d-1)*ddotmu_chi(1)
           opacity_DR=dotmu_DR(1)+(d-1)*ddotmu_DR(1)
!!!
           stop 'dark thermo out of bounds'
        else if (i >= nthermo_dark) then
           cs2b=cs2(nthermo_dark)+(d+i-nthermo_dark)*dcs2(nthermo_dark)
           opacity=dotmu(nthermo_dark)+(d-nthermo_dark)*ddotmu(nthermo_dark)
           opacity_chi=dotmu_chi(nthermo_dark)+(d-nthermo_dark)*ddotmu_chi(nthermo_dark)
           opacity_DR=dotmu_DR(nthermo_dark)+(d-nthermo_dark)*ddotmu_DR(nthermo_dark)
           if (present(dopacity)) then
              dopacity = 0
              stop 'dark thermo: should not happen'
           end if
        else
           !Cubic spline interpolation.
           ...
           opacity=dotmu(i)+d*(ddotmu(i)+d*(3*(dotmu(i+1)-dotmu(i)) &
                ...
                +2*(dotmu(i)-dotmu(i+1)))))
           opacity_chi=dotmu_chi(i)+d*(ddotmu_chi(i)+d*(3*(dotmu_chi(i+1)-dotmu_chi(i)) &
                -2*ddotmu_chi(i)-ddotmu_chi(i+1)+d*(ddotmu_chi(i)+ddotmu_chi(i+1) &
                +2*(dotmu_chi(i)-dotmu_chi(i+1)))))
           opacity_DR=dotmu_DR(i)+d*(ddotmu_DR(i)+d*(3*(dotmu_DR(i+1)-dotmu_DR(i)) &
                -2*ddotmu_DR(i)-ddotmu_DR(i+1)+d*(ddotmu_DR(i)+ddotmu_DR(i+1) &
                +2*(dotmu_DR(i)-dotmu_DR(i+1)))))
           ...
           if (present(dopacity_chi)) then
              dopacity_chi=(ddotmu_chi(i)+d*(dddotmu_chi(i)+d*(3*(ddotmu_chi(i+1)  &
                   -ddotmu_chi(i))-2*dddotmu_chi(i)-dddotmu_chi(i+1)+d*(dddotmu_chi(i) &
                   +dddotmu_chi(i+1)+2*(ddotmu_chi(i)-ddotmu_chi(i+1))))))/(tau*dlntau)
              
           end if

           if (present(dopacity_DR)) then
              dopacity_DR=(ddotmu_DR(i)+d*(dddotmu_DR(i)+d*(3*(ddotmu_DR(i+1)  &
                   -ddotmu_DR(i))-2*dddotmu_DR(i)-dddotmu_DR(i+1)+d*(dddotmu_DR(i) &
                   +dddotmu_DR(i+1)+2*(ddotmu_DR(i)-ddotmu_DR(i+1))))))/(tau*dlntau)
      ...
      subroutine inithermo_dark(taumin,taumax)
        !  Compute and save DM and DR opacities and DM
        !  sound speed
        use constants
        use precision
        use ModelParams
        ...
        real(dl) tau,a,a2
        real(dl) DM_DR_z,DM_DR_z0
        real(dl) adot,tg0,ahalf,adothalf,thomc,thomc0,etc,a2t
        real(dl) a_D_norm
        real(dl) dtbdla
        integer  i,j1,j2,iv,ns,i_DM_DR
        real(dl) spline_data_dark(nthermo_dark)
        real(dl) trad
        real(dl) t_switch, a_switch
        real(dl) i_DM_DR_db 
        real(dl) dtauda  !diff of tau wrt a and integration
        
        !To solve the Friedman equation
        external dtauda
        
        Maxtau=taumax

        !The normalization scale factor for the opacity and heating coefficients
        a_D_norm = 1.0_dl/(1.0_dl + CP%z_D_norm)
        
        !Initialize the important epochs
        tight_tau_dark = 0.0d0
        drag_tau_dark = 0.0d0
        tau_dec_dark = 0.0d0
        end_drag_dark = 0.0d0
        ...
        thomc = 0.0 
        do i_DM_DR = 1,CP%DM_DR_highest_n + 1
           thomc = thomc + CP%omDRh2*CP%d_DM_DR(i_DM_DR)*a_D_norm**(i_DM_DR-1)/adot0/a0**(i_DM_DR)
        end do

        !Allow for non equal DM and DR temperatures
        !Note that a0*thomc is \Gamma_heat/H at tauminn (where Gamma_heat is the heating rate and H is the conformal Hubble rate)
        if (a0*thomc > 1.0d3) then
           tb(1) = CP%t_dark/a0
        else
           tb(1)= (a0*thomc/(1.0d0 + a0*thomc))*(CP%t_dark/a0)
        end if
        
        !ETHOSV1-FIX(#5): seed the adiabatic-switch state so the low-heating branch
        !below never reads undefined values (triggered when d_n is zero or tiny)
        t_switch = tb(1)
        a_switch = a0

        !Initial dark matter sound speed and DM and DR opacities
        cs2(1) = 4._dl/3._dl*(k_B*tb(1))/(CP%m_DM*C*C)
        dotmu(1) = 0.0
        dotmu_chi(1) = 0.0
        dotmu_DR(1) = 0.0
        do i_DM_DR = 1,CP%DM_DR_highest_n + 1
           i_DM_DR_db = dble(i_DM_DR)
           dotmu(1) = dotmu(1) + CP%omegac*CP%f_sidm*(CP%H0/100)**2*CP%a_DM_DR(i_DM_DR)* &
                ((1.0_dl + DM_DR_z0)/(1.0_dl + CP%z_D_norm))**(i_DM_DR_db-1.0d0)
           dotmu_chi(1) = dotmu_chi(1) + CP%omDRh2*CP%a_DM_DR(i_DM_DR)*(4.0d0/3.0d0)* &
                (1.0_dl + DM_DR_z0)**(i_DM_DR_db)/(1.0_dl + CP%z_D_norm)**(i_DM_DR_db-1.0d0)
           dotmu_DR(1) = dotmu_DR(1) + CP%omDRh2*CP%b_DR_DR(i_DM_DR)*((1.0_dl + DM_DR_z0)/ &
                (1.0_dl + CP%z_D_norm))**(i_DM_DR_db-1.0d0)
        end do

        !FYCR: open file to write sound speed and opacity        
        open(unit=25,file=trim(CP%outroot_dark)//'kappadot_cs2.dat',status='replace')
        ...
        do i=2,nthermo_dark
           ...
           a=a0+adot0*dtau
           DM_DR_z = 1.0_dl/a-1.0_dl
           a2=a*a
           adot=1/dtauda(a)
           a=a0+2._dl*dtau/(1._dl/adot0+1._dl/adot)    
     
           !  Dark Matter temperature evolution: adiabatic except for DR heating.
           ...
           thomc = 0.0 
           do i_DM_DR = 1,CP%DM_DR_highest_n + 1
               thomc = thomc + CP%omDRh2*CP%d_DM_DR(i_DM_DR)*a_D_norm**(i_DM_DR-1)/adothalf/ahalf**(i_DM_DR)
           end do
            
           !ETHOSV1-FIX(#6): take the exact thomc->0 limit instead of dividing by zero
           if (thomc > 0._dl) then
              etc=exp(-thomc*(a-a0))
              a2t=a0*a0*(tb(i-1)-tg0)*etc-CP%t_dark/thomc*(1._dl-etc)
           else
              etc=1._dl
              a2t=a0*a0*(tb(i-1)-tg0)-CP%t_dark*(a-a0)
           end if
           
           !DM temperature (Note switch to adiabatic approximation at late time for numerical stability)
           if (ahalf*thomc > 1.0d-4 ) then
              tb(i)=CP%t_dark/a+a2t/(a*a)
              dtbdla=-2._dl*tb(i)-thomc*adothalf/adot*(a2t/a)
           elseif ((ahalf*thomc <= 1.0d-4) .and. (ahalf*thomc > 5.0d-5)) then
              tb(i)=CP%t_dark/a+a2t/(a*a)
              dtbdla=-2._dl*tb(i)-thomc*adothalf/adot*(a2t/a)
              t_switch = tb(i)
              a_switch = a
           else
              tb(i) = t_switch*(a_switch/a)**2
              dtbdla=-2._dl*tb(i)
           end if

           !DM sound speed squared (over c**2).
           !ETHOSV1-FIX(#14): tb=0 (fully decoupled dark sector, no heating) made
           !dtbdla/tb = 0/0 = NaN; the physical limit of a cold decoupled fluid is cs2=0
           if (tb(i) > 0._dl) then
              cs2(i)=(k_B*tb(i))/(CP%m_DM*C*C)*(1._dl-dtbdla/tb(i)/3._dl)
           else
              cs2(i)=0._dl
           end if
           
           ! Calculation of the opacities
           dotmu(i) = 0.0
           dotmu_chi(i) = 0.0
           dotmu_DR(i) = 0.0
           do i_DM_DR = 1,CP%DM_DR_highest_n + 1
              i_DM_DR_db = dble(i_DM_DR)
              dotmu(i) = dotmu(i) + CP%omegac*CP%f_sidm*(CP%H0/100)**2*CP%a_DM_DR(i_DM_DR)* &
                   ((1.0_dl + DM_DR_z)/(1.0_dl + CP%z_D_norm))**(i_DM_DR-1)
              dotmu_chi(i) = dotmu_chi(i) + CP%omDRh2*CP%a_DM_DR(i_DM_DR)*(4.0d0/3.0d0)* &
                   (1.0_dl + DM_DR_z)**(i_DM_DR)/(1.0_dl + CP%z_D_norm)**(i_DM_DR-1)
              dotmu_DR(i) = dotmu_DR(i) + CP%omDRh2*CP%b_DR_DR(i_DM_DR)*((1.0_dl + DM_DR_z)/ &
                   (1.0_dl + CP%z_D_norm))**(i_DM_DR-1)
           end do

           ! Write the results to a file
           write(unit=25,fmt=56) tau, 1.d0/a-1.d0, cs2(i), tb(i),trad,dotmu(i),dotmu_chi(i),dotmu_DR(i),adot/a
           56        format(E14.8,1X,E14.8,1X,E14.8,1X,E14.8,1X,E14.8,1X,E14.8,1X,E14.8,1X,E14.8,1X,E14.8)

           !ETHOSV1-FIX(#7): rewritten as products so zero opacity is well defined
           !(1/(tau*x) > c  <=>  tau*x < 1/c for tau,x > 0; zero x now counts as decoupled)
           !Determine tight-coupling switch for DR
           if (tight_tau_dark==0 .and. tau*dotmu(i) < 200._dl) tight_tau_dark = tau

           !Determine tight-coupling switch for DM
           if (drag_tau_dark==0 .and. tau*dotmu_chi(i) < 200._dl) drag_tau_dark = tau

           !For info, determine the DR decoupling redshift
           if (tau_dec_dark ==0 .and. 0.99_dl*tau*dotmu(i) < 1._dl) tau_dec_dark = tau

           !For info, determine the end of the drag epoch
           if (end_drag_dark ==0 .and. 0.99_dl*tau*dotmu_chi(i) < 1._dl) end_drag_dark = tau

           ! Pass the values to the next step
           a0=a
           tau01=tau
           adot0=adot
        end do !i
        ...
        print*, 'DR Tight Coupling Switches off at tau = ',tight_tau_dark
        print*, 'DM Tight Coupling Switches off at tau = ',drag_tau_dark
        print*, 'Dark Radiation Decouples at tau       = ',tau_dec_dark
        print*, 'Dark Matter Decouples at tau          = ',end_drag_dark 
        
        !Perform the spline interpolation
        call splini(spline_data_dark,nthermo_dark)
        call splder(cs2,dcs2,nthermo_dark,spline_data_dark)
        ...
        call splder(dotmu_chi,ddotmu_chi,nthermo_dark,spline_data_dark)
        call splder(ddotmu_chi,dddotmu_chi,nthermo_dark,spline_data_dark)  
        call splder(dddotmu_chi,ddddotmu_chi,nthermo_dark,spline_data_dark)

        call splder(dotmu_DR,ddotmu_DR,nthermo_dark,spline_data_dark)
        call splder(ddotmu_DR,dddotmu_DR,nthermo_dark,spline_data_dark)  
        call splder(dddotmu_DR,ddddotmu_DR,nthermo_dark,spline_data_dark)
    ...
    if (CP%DerivedParameters) then
        ...
        ThermoDerivedParams( derived_zEQ ) = (grhob+grhoc+grhoc_cdm)/(grhog+grhog_dark+&
             grhornomass+sum(grhormass(1:CP%Nu_mass_eigenstates))) -1
