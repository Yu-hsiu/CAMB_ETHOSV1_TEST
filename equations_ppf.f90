    ...
    ! Sep 2014: FYC: Updated for interacting DM and DR
...
1   if (khi-klo.gt.1) then
    ...
    !  8*pi*G*rho*a**4.
    !FYCR-DM-DR
    !Add contribution from SIDM and DR
    grhoa2=grhok*a2+(grhoc+grhob+grhoc_cdm)*a+grhog+grhornomass+grhog_dark
    ...
    !FYCR-DM-DR
    !Was 5, but now include SIDM velocity and Plain CDM perturbations
    ...
    type EvolutionVars
        ...
        !FYCR-DM-DR
        integer gdark_ix !Index of the dark photon hierarchy
        ...
        !FYCR-DM-DR
        !Dark-sector variables 
        real(dl) TightSwitchoffTime_dark
        integer lmaxg_dark
        real(dl) pig_dark, pigdot_dark
        logical TightCoupling_dark
        logical no_dark_phot_multpoles
    ...
    !FYCR-DM-DR
    real(dl) cs2_dark,opacity_dark,opacity_chi_dark,opacity_DR_dark
    ...
    !FYCR-DM-DR
    real(dl) tau_switch_no_dark_phot_multpoles,tau_switch_dark, tau_check_dark
    ...
    !FYCR-DM-DR
    tau_switch_no_dark_phot_multpoles= noSwitch
    ...
    if (DoLateRadTruncation) then
        ...
        !FYCR-DM-DR
        !When to switch off the DR Hierarchy
        !  if (.not. EV%no_dark_phot_multpoles .and. (.not.CP%WantCls .or. EV%k_buf>0.03*AccuracyBoost)) &
        !       tau_switch_no_dark_phot_multpoles =max(15/EV%k_buf,matter_verydom_tau)*AccuracyBoost 
    end if
    
    !FYCR-DM-DR
    next_switch = min(tau_switch_ktau, tau_switch_nu_massless,EV%TightSwitchoffTime_dark,EV%TightSwitchoffTime,&
         tau_switch_nu_massive, tau_switch_no_nu_multpoles, tau_switch_no_phot_multpoles, &
         tau_switch_no_dark_phot_multpoles, tau_switch_nu_nonrel,noSwitch)
    ...
    if (next_switch < tauend) then
        ...
        if (next_switch == EV%TightSwitchoffTime_dark) then 
           ...
           y(EV%gdark_ix+2) = EV%pig_dark
           call thermo_dark(tau,cs2_dark,opacity_dark,opacity_chi_dark,opacity_DR_dark)
           !ETHOSV1-FIX(#15): with zero dark opacity the TC closure y3 = 3/7*y2*k/(alpha_3*kappa')
           !is 0/0 at the switch (fires at tau~taustart for every k, so the whole run went NaN;
           !-ffast-math builds folded the 0-numerator away, masking this). No tight coupling ever
           !existed here, so start the octopole from its free-streaming value.
           if (opacity_dark > 0._dl) then
              y(EV%gdark_ix+3) = 3./7*y(EV%gdark_ix+2)*EV%k_buf/(CP%alpha_DM_DR_l(3)*opacity_dark)
           else
              y(EV%gdark_ix+3) = 0._dl
           end if
    ...
    !FYCR-DM-DR
    if (.not. EV%no_dark_phot_multpoles) then
       !DR multipoles
       EV%gdark_ix = neq + 1
       if (EV%TightCoupling_dark) then
          neq = neq + 2
       else 
          neq = neq + (EV%lmaxg_dark + 1)
       end if
    end if
    maxeq = maxeq +  (EV%lmaxg_dark + 1)
    ...
    !FYCR-DM-DR
    !Rearrange DR array when changing lmax
    if (.not. EV%no_dark_phot_multpoles .and. .not. EVout%no_dark_phot_multpoles) then 
       if (EV%TightCoupling_dark .or. EVOut%TightCoupling_dark) then
          lmax=1
       else 
          lmax = min(EV%lmaxg_dark,EVout%lmaxg_dark)
       end if
       yout(EVout%gdark_ix:EVout%gdark_ix+lmax)=y(EV%gdark_ix:EV%gdark_ix+lmax)          
    end if
    ...
    if (CP%WantScalars) then
        ...
        !FYCR-DM-DR
        !Turn on dark tight-coupling
        EV%TightCoupling_dark=.true.
        !ETHOSV1-FIX(#11): seed tight-coupling values so they are defined even if the
        !dark tight-coupling era ends before the first derivs evaluation
        EV%pig_dark=0._dl
        EV%pigdot_dark=0._dl
        EV%no_phot_multpoles =.false.
        !FYCR-DM-DR
        !Evolve DR multipoles to start
        EV%no_dark_phot_multpoles = .false.
        ...
        if (EV%TransferOnly) then
            ...
            !FYCR-DM-DR
            EV%lmaxg_dark = max(nint(8*lAccuracyBoost),3)
        end if
        if (CP%Transfer%high_precision) then
            if (HighAccuracyDefault) then
                EV%lmaxnr=max(nint(45*lAccuracyBoost),3)
                !FYCR-DM-DR
                EV%lmaxg_dark = max(nint(16*lAccuracyBoost),3)
            else
                EV%lmaxnr=max(nint(30*lAccuracyBoost),3)
                !FYCR-DM-DR
                EV%lmaxg_dark = max(nint(12*lAccuracyBoost),3)
            endif
        ...
        if (CP%closed) then
            ...
            !FYCR-DM-DR
            EV%lmaxg_dark=min(EV%lmaxg_dark, EV%FirstZerolForBeta-1) 
            EV%lmaxgpol=min(EV%lmaxgpol, EV%FirstZerolForBeta-1)
        end if

        EV%poltruncfac=real(EV%lmaxgpol,dl)/max(1,(EV%lmaxgpol-2))
        !FYCR-DM-DR
        EV%MaxlNeeded=max(EV%lmaxg,EV%lmaxg_dark,EV%lmaxnr,EV%lmaxgpol,EV%lmaxnu)
    ...
    real(dl) grhoc_cdm_t,grhog_dark_t
    real(dl) qgdot,pigdot,pirdot,vbdot,dgrho
    real(dl) a,a2,dz,z,clxc,clxb,vb,clxg,qg,pig,clxr,qr,pir
    !FYCR-DM-DR
    !Dark perturbation variables
    real(dl) qg_dark, vc_dark, clxg_dark,cs2b_dark,opac_dark,opac_chi_dark,dopac_dark,vcdot_dark
    real(dl) pigdot_dark, pig_dark,qgdot_dark,clxc_cdm,dopac_chi_dark,opac_DR_dark,dopac_DR_dark
    ...
    grhoc_t=grhoc/a
    !FYCR-DM-DR
    grhoc_cdm_t=grhoc_cdm/a
    grhor_t=grhornomass/a2
    grhog_t=grhog/a2
    !FYCR-DM-DR
    grhog_dark_t=grhog_dark/a2

    !  8*pi*a*a*SUM[rho_i*clx_i] add radiation later
    !FYCR-DM-DR
    dgrho=grhob_t*clxb+grhoc_t*clxc+grhoc_cdm_t*clxc_cdm

    !  8*pi*a*a*SUM[(rho_i+p_i)*v_i]
    !FYCR-DM-DR
    dgq=grhob_t*vb + grhoc_t*vc_dark
    ...
    !FYCR-DM-DR
    grho=grhob_t+grhoc_t+grhoc_cdm_t+grhor_t+grhog_t+grhov_t+grhog_dark_t
    gpres=(grhog_t+grhor_t+grhog_dark_t)/3+grhov_t*w_eff
    ...
    !FYCR-DM-DR
    if (EV%no_dark_phot_multpoles) then
       z=(0.5_dl*dgrho/k + etak)/adotoa 
       dz= -adotoa*z - 0.5_dl*dgrho/k
       call thermo_dark(tau,cs2b_dark,opac_dark,opac_chi_dark,opac_DR_dark, dopac_dark,dopac_chi_dark,dopac_DR_dark)
       clxg_dark=-4*dz/k -4/k*opac_dark*(vc_dark+z)
       qg_dark=-4._dl/3*z
       pig_dark=0
       pigdot_dark=0
       qgdot_dark = -4*dz/k
       ...
       if (EV%TightCoupling_dark) then
          pig_dark = EV%pig_dark
          pigdot_dark=EV%pigdot_dark
       else
          pig_dark =y(EV%gdark_ix+2)
          pigdot_dark=yprime(EV%gdark_ix+2)
       end if
       clxg_dark=y(EV%gdark_ix)
       qg_dark  =y(EV%gdark_ix+1)
       qgdot_dark =yprime(EV%gdark_ix+1)
    end if
    
    !FYCR-DM-DR
    dgrho = dgrho + grhog_t*clxg+grhor_t*clxr + grhog_dark_t*clxg_dark
    dgq   = dgq   + grhog_t*qg+grhor_t*qr  + grhog_dark_t*qg_dark
    dgpi  = dgpi  + grhor_t*pir + grhog_t*pig  + grhog_dark_t*pig_dark
    ...
    !FYCR-DM-DR
    pidot_sum =  pidot_sum + grhog_t*pigdot + grhor_t*pirdot + grhog_dark_t*pigdot_dark
    ...
    i_clxc_cdm=13
    integer, parameter :: i_max = i_clxc_cdm
    ...
    !FYCR-DM-DR
    EV%TightSwitchoffTime_dark = min(tight_tau_dark,Thermo_OpacityToTime_dark(EV%k_buf/ep)) 
    ...
    !FYCR-DM-DR
    om = (grhob+grhoc)/sqrt(3*(grhog+grhonu+grhog_dark))
    omtau=om*tau
 
    !FYCR-DM-DR
    if ( tau > tau_dec_dark) then !Then dark photon are free streaming too
       Rv=(grhonu+grhog_dark)/(grhonu+grhog+grhog_dark)
       Rg = 1.0d0 - Rv
    else
       Rv = (grhonu)/(grhonu+grhog+grhog_dark)
       Rg = (grhog)/(grhonu+grhog+grhog_dark)
    end if
    ...
    !FYCR-DM-DR
    initv(1,i_clxc_cdm)=initv(1,i_clxb)
    ...
    ! SIDM
    y(3)=InitVec(i_clxc)
    if ( tau > tau_dec_dark) then  !Free Streaming solution
       y(4) = 0.75_dl*InitVec(i_qr)
    else   !Tightly-Coupled
       y(4) = 0.75_dl*InitVec(i_qg)
    end if
    ...
    !FYCR-DM-DR
    !  Dark radiation
    y(EV%gdark_ix) = InitVec(i_clxg)
    if ( tau > tau_dec_dark) then  !Free Streaming solution
       y(EV%gdark_ix+1) = InitVec(i_qr)
       y(EV%gdark_ix+2) = InitVec(i_pir)
    else
       y(EV%gdark_ix+1) = InitVec(i_qg)
    end if
    ...
    !FYCR-DM-DR
    !Here clxc stands for the interacting DM component, whereas
    !clxc_cdm stands for the standard noninteracting CDM (if present)
    real(dl) clxc_cdm,clxg_dark
    ...
    !FYCR-DM-DR
    Arr(Transfer_cdm) = clxc_cdm/k2
    Arr(Transfer_b) = clxb/k2
    Arr(Transfer_g) = clxg/k2
    Arr(Transfer_r) = clxr/k2
    !FYCR-DM-DR
    Arr(Transfer_sidm) = clxc/k2
    Arr(Transfer_g_dark) = clxg_dark/k2
    ...
    !FYCR-DM-DR
    dgrho = dgrho+(clxc*grhoc + clxc_cdm*grhoc_cdm + clxb*grhob)/a
    grho =  grho+(grhoc+grhoc_cdm+grhob)/a
    ...
    !  FYCR: Include DM-DR interaction
    use ThermoData
    !FYCR-DM-DR
    use ThermoData_dark
    use MassiveNu
    ...
    !FYCR-DM-DR
    real(dl) clxc_cdm,clxc_cdmdot,grhoc_cdm_t
    real(dl) q,aq,v
    real(dl) G11_t,G30_t, wnu_arr(max_nu)

    !FYCR-DM-DR
    real(dl) opacity_dark,opacity_chi_dark,opacity_DR_dark,dopacity_dark,dopacity_chi_dark,dopacity_DR_dark
    real(dl) cs2_dark, clxg_dark, qg_dark, pig_dark
    real(dl) clxgdot_dark,pb43_dark,slip_dark, pigdot_dark, qgdot_dark
    real(dl) vc_dark,vcdot_dark,grhog_dark_t,alpha_l,slip_dark2
    ...
    !  Compute expansion rate from: grho 8*pi*G*rho*a**2
    ...
    !FYCR-DM-DR
    !  Get dark sound speed and opacity.
    if (EV%TightCoupling_dark .or. tau < min(drag_tau_dark,1.0d0/k)) then
       call thermo_dark(tau,cs2_dark,opacity_dark,opacity_chi_dark,opacity_DR_dark, &
            dopacity_dark,dopacity_chi_dark,dopacity_DR_dark)
    else
       call thermo_dark(tau,cs2_dark,opacity_dark,opacity_chi_dark,opacity_DR_dark)        
    end if

    !FYCR-DM-DR
    gpres=(grhor_t+grhog_t+grhog_dark_t)/3._dl
    ...
    if (EV%no_dark_phot_multpoles) then
       if (.not. EV%no_nu_multpoles) then
          z=(0.5_dl*dgrho/k + etak)/adotoa 
          dz= -adotoa*z - 0.5_dl*dgrho/k
          clxg_dark=-4*dz/k-4/k*opacity_dark*(vc_dark+z)
          qg_dark=-4._dl/3*z
       else
          clxg_dark=clxr-4/k*opacity_dark*(vc_dark+z)
          qg_dark=qr          
       end if
       pig_dark=0    
    else
       clxg_dark=ay(EV%gdark_ix)
       qg_dark=ay(EV%gdark_ix+1)
       if (.not. EV%TightCoupling_dark) pig_dark=ay(EV%gdark_ix+2)
    end if
    ...
    !FYCR-DM-DR
    ! Here, this is the ratio of the opacities
    !ETHOSV1-FIX(#8): guard against 0/0 when the dark sector is fully decoupled (all a_n=0)
    if (opacity_dark > 0._dl) then
       pb43_dark= opacity_chi_dark/opacity_dark
    else
       pb43_dark= 0._dl
    end if
    ...
    !FYCR-DM-DR
    !  DR equation of motion
    clxgdot_dark=-k*(4._dl/3._dl*z+qg_dark)
    ...
    if (EV%TightCoupling_dark) then
       ...
       if (EV%TightCoupling) pig = 32._dl/45/opacity*k*(sigma+vb)
       !ETHOSV1-FIX(#8): with zero dark opacity there is no tight coupling physics;
       !fall back to the free (decoupled) DM equation instead of dividing by zero
       if (opacity_dark > 0._dl) then
          pig_dark = 8._dl/15/opacity_dark/CP%alpha_DM_DR_l(2)*k*(sigma+vc_dark)

          !  First-order approximation to DM-DR slip


          slip_dark = - ((dopacity_dark + dopacity_chi_dark)/(opacity_dark+opacity_chi_dark)) * (vc_dark-3._dl/4*qg_dark) &
               +(-adotdota*vc_dark-k/2*adotoa*clxg_dark +k*(cs2_dark*clxcdot-clxgdot_dark/4))/(opacity_dark + opacity_chi_dark)

          !  Use tight-coupling approximation for vc_dark
          !  zeroth order approximation to vcdot_dark + the pig_dark term

          vcdot_dark=(-adotoa*vc_dark+cs2_dark*k*clxc  &
               +k/4*pb43_dark*(clxg_dark-2*EV%Kf(1)*pig_dark))/(1+pb43_dark)

          vcdot_dark=vcdot_dark+pb43_dark/(1.0d0+pb43_dark)*slip_dark
       else
          pig_dark = 0._dl
          slip_dark = 0._dl
          vcdot_dark = -adotoa*vc_dark+cs2_dark*k*clxc
       ...
       EV%pig_dark = pig_dark

    elseif((.not. EV%TightCoupling_dark) .and. tau < min(drag_tau_dark,1.0_dl/k)) then
       ! This for the period between DR last scattering and the end of the DM drag epoch, when
       ! the DM equation can still be stiff
       !ETHOSV1-FIX(#9): adotdota was previously read uninitialized in this branch
       adotdota=(adotoa*adotoa-gpres)/2
       !ETHOSV1-FIX(#16): same family as #8 -- the stiff-branch slip divides by opacity_dark;
       !with zero dark opacity there is no drag and the slip is identically zero
       if (opacity_dark > 0._dl) then
          slip_dark = - (2*adotoa/(1+pb43_dark) + dopacity_dark/opacity_dark)* (vc_dark-3._dl/4*qg_dark) &
               +(-adotdota*vc_dark-k/2*adotoa*clxg_dark +k*(cs2_dark*clxcdot-clxgdot_dark/4))/(opacity_dark*(1+pb43_dark))
       else
          slip_dark = 0._dl
       end if
       ...
       ! Standard equation for DM velocity when non-stiff
       vcdot_dark=-adotoa*vc_dark+cs2_dark*k*clxc- opacity_chi_dark*(vc_dark-3._dl/4*qg_dark)
    end if
    
    ayprime(4)=vcdot_dark
    ...
    if (EV%TightCoupling) then
        ...
        if (second_order_tightcoupling) then
           ...
           !FYCR-DM-DR
            dgs = grhog_t*pig+grhor_t*pir+grhog_dark_t*pig_dark
    ...
    if (.not. EV%no_dark_phot_multpoles) then
       ...
       ! Dark-Photon equations of motion
       ayprime(EV%gdark_ix)=clxgdot_dark
       !ETHOSV1-FIX(#10): when decoupled (pb43_dark=0) the drag term vanishes;
       !use the free-streaming form instead of dividing by zero
       if (pb43_dark > 0._dl) then
          qgdot_dark=4._dl/3*(-vcdot_dark-adotoa*vc_dark+cs2_dark*k*clxc)/pb43_dark &
               +EV%denlk(1)*clxg_dark-EV%denlk2(1)*pig_dark
       else
          qgdot_dark=EV%denlk(1)*clxg_dark-EV%denlk2(1)*pig_dark
       end if
       ayprime(EV%gdark_ix+1)=qgdot_dark
       
       !  Use explicit equations for dark-photon moments if appropriate       
       if (.not. EV%tightcoupling_dark) then
          ix= EV%gdark_ix+2
          if (EV%lmaxg_dark>2) then 
             pigdot_dark=EV%denlk(2)*qg_dark-EV%denlk2(2)*ay(ix+1)-CP%alpha_DM_DR_l(2)*opacity_dark*pig_dark &
                  +8._dl/15._dl*k*sigma
             ayprime(ix)=pigdot_dark
             do  l=3,EV%lmaxg_dark-1
                ix=ix+1
                !FYCR-DM-DR put in the alpha_l coefficients
                if( l <= CP%DM_DR_highest_l) then
                   alpha_l = CP%alpha_DM_DR_l(l)
                else
                   alpha_l = CP%alpha_DM_DR_l(CP%DM_DR_highest_l)
                end if
                ayprime(ix)=(EV%denlk(l)*ay(ix-1)-EV%denlk2(l)*ay(ix+1))-alpha_l*opacity_dark*ay(ix)
             end do
             ix=ix+1
             l=l+1
             if( l <= CP%DM_DR_highest_l) then
             ...
             !  Truncate the dark-photon moment expansion
             ayprime(ix)=k*ay(ix-1)-(EV%lmaxg_dark+1)*cothxor*ay(ix) -alpha_l*opacity_dark*ay(ix)
          else !closed case
             pigdot_dark=EV%denlk(2)*qg_dark-CP%alpha_DM_DR_l(2)*opacity_dark*(pig_dark) +8._dl/15._dl*k*sigma
             ayprime(ix)=pigdot_dark
          endif
       end if
    end if
