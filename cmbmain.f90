    ...
    !     Modified for Interacting Dark Matter within the ETHOS framework by Francis-Yan Cyr-Racine, 2016
    ...
    !FYCR-DM-DR
    !needed to chech whether we start at early enough time to capture DR decoupling
    real(dl)  taumin_for_check
    ...
    function GetTauStart(q)
      ...
      !FYCR-DM-DR
      !Make sure to start when dark plasma is tightly-coupled, unless the DR is free-streaming already
      if ( taustart > tight_tau_dark) then
         if(tight_tau_dark < 1.0d-6) then
            taustart = 0.1*drag_tau_dark
         else 
            taustart = 0.1*tight_tau_dark
         end if
      end if
      
      if(taustart .lt. taumin_for_check) taustart =  taumin_for_check
    ...
    !FYCR-DM-DR
    real(dl) taumin_dark
    real(dl)  min_i_DM_DR_db
    integer i_DM_DR, min_i_DM_DR
    ...
    !FYCR-Make sure we start early enough to include proper dark physics
    !Assume radiation domination since this is only to get a rough idea of the 
    !decoupling tau
    
    ! Pick out the slowest growing contribution to the opacity (which is assumed to control decoupling)
    !ETHOSV1-FIX(#12): initialize so a fully decoupled model (all a_n=0) skips the
    !dark-epoch taumin adjustment instead of indexing a_DM_DR with an undefined value
    min_i_DM_DR = 0
    do i_DM_DR = 1,CP%DM_DR_highest_n + 1
       if (CP%a_DM_DR(i_DM_DR) > 1.0d-100) then
          min_i_DM_DR = i_DM_DR
          exit
       end if
    end do

    ! If min_i_DM_DR = 1, then the DM opacity tracks the Hubble rate during rad domination
    ! and the DM drag epoch only happens after matter-radiation equality
    ! In this case, the DR (assuming no self-interaction) is decoupled at early times
    ! but it can recouple at late times
    ! If min_i_DM_DR = 2, then the DR opacity tracks the Hubble rate and it cannot really
    ! free stream until matter-radiation equality
    min_i_DM_DR_db = dble(min_i_DM_DR)
    if (min_i_DM_DR > 2) then
       taumin_dark = (CP%omegac*CP%f_sidm*(CP%H0/100)**2*CP%a_DM_DR(min_i_DM_DR))**(1.0d0/(min_i_DM_DR_db-2.0d0)) &
            *(1.0d0/adotrad)**((min_i_DM_DR_db-1.0d0)/(min_i_DM_DR_db-2.0d0))
       if (taumin > taumin_dark) taumin = taumin_dark
    else if (min_i_DM_DR == 2) then
       taumin_dark = ((CP%omDRh2*((1.0d0+ min_i_DM_DR_db)/3.0d0) &
            *CP%a_DM_DR(min_i_DM_DR))**(1.0d0/(min_i_DM_DR_db-1.0d0)))*(1.0d0/adotrad)**(min_i_DM_DR_db/(min_i_DM_DR_db-1.0d0))
        if (taumin > taumin_dark) taumin = taumin_dark
    end if
    
    !print*, 'min_i_DM_DR =',min_i_DM_DR,' tau_min_dark = ',taumin_dark, 'taumin =', taumin
    
    taumin_for_check = taumin
    ...
    !FYCR-DM-DR
    real(dl) a,dgrho,zevol,dgq,dgs,sigma,psi
    real(dl) convert_delta, convert_theta,phi,a2
    external dtauda
    ...
    if (fixq/=0._dl) then
       ...
       print*, 'opening file'
       open(unit=35,file='spectra/SIDM_evol_var_k_100.dat',status='replace')
       ...
       do j=0,10000
          ...
          a=y(1)
          a2 = a*a
          adotoa=sqrt((grhob/a + grhoc/a + grhoc_cdm/a + grhog/a2 + grhornomass/a2 + grhog_dark/a2 + grhov*a2)/3.0d0)
          dgrho=grhob/a*y(5)+grhoc/a*y(3)+ grhoc_cdm/a*y(7) + grhog/a**2*y(EV%g_ix)+ &
               grhornomass/a**2*y(EV%r_ix)+grhog_dark/a**2*y(EV%gdark_ix)
          dgq=grhob/a*y(6) + grhoc/a*y(4)+ grhog/a**2*y(EV%g_ix+1)+ grhornomass/a**2*y(EV%r_ix+1) &
               +grhog_dark/a**2*y(EV%gdark_ix+1)
          ...
          if(EV%TightCoupling_dark .and. EV%TightCoupling) then
             dgs = grhog/a**2*EV%pig+grhornomass/a**2*y(EV%r_ix+2)+grhog_dark/a**2*EV%pig_dark
             psi = phi - dgs/EV%q2 
             write (35,'(19E18.9)') tau,y(1),y(3)-3.0_dl*convert_delta,EV%q*y(4)+convert_theta,&
                  y(5)-3.0_dl*convert_delta,EV%q*y(6)+convert_theta,y(7)-3.0_dl*convert_delta, &
                  0.75_dl*(y(EV%gdark_ix)-4.0_dl*convert_delta),0.75_dl*EV%q*y(EV%gdark_ix+1)+convert_theta,EV%pig_dark, &
                  0.75_dl*(y(EV%g_ix)-4.0_dl*convert_delta),0.75_dl*EV%q*y(EV%g_ix+1)+convert_theta,EV%pig, &
                  0.75_dl*(y(EV%r_ix)-4.0_dl*convert_delta),phi,psi,y(2)/EV%q,sigma,adotoa
          elseif(.not.EV%TightCoupling_dark .and. EV%TightCoupling) then
             dgs = grhog/a**2*EV%pig+grhornomass/a**2*y(EV%r_ix+2)+grhog_dark/a**2*y(EV%gdark_ix+2)
             psi = phi - dgs/EV%q2       
             write (35,'(19E18.9)') tau,y(1),y(3)-3.0_dl*convert_delta,EV%q*y(4)+convert_theta,&
                  y(5)-3.0_dl*convert_delta,EV%q*y(6)+convert_theta,y(7)-3.0_dl*convert_delta, &
                  0.75_dl*(y(EV%gdark_ix)-4.0_dl*convert_delta),0.75_dl*EV%q*y(EV%gdark_ix+1)+convert_theta,y(EV%gdark_ix+2), &
                  0.75_dl*(y(EV%g_ix)-4.0_dl*convert_delta),0.75_dl*EV%q*y(EV%g_ix+1)+convert_theta,EV%pig, &
                  0.75_dl*(y(EV%r_ix)-4.0_dl*convert_delta),phi,psi,y(2)/EV%q,sigma,adotoa
          elseif(EV%TightCoupling_dark .and. .not.EV%TightCoupling) then
             dgs = grhog/a**2*y(EV%g_ix+2)+grhornomass/a**2*y(EV%r_ix+2)+grhog_dark/a**2*EV%pig_dark
             psi = phi - dgs/EV%q2 
             write (35,'(19E18.9)') tau,y(1),y(3)-3.0_dl*convert_delta,EV%q*y(4)+convert_theta,&
                  y(5)-3.0_dl*convert_delta,EV%q*y(6)+convert_theta,y(7)-3.0_dl*convert_delta, &
                  0.75_dl*(y(EV%gdark_ix)-4.0_dl*convert_delta),0.75_dl*EV%q*y(EV%gdark_ix+1)+convert_theta,EV%pig_dark, &
                  0.75_dl*(y(EV%g_ix)-4.0_dl*convert_delta),0.75_dl*EV%q*y(EV%g_ix+1)+convert_theta,y(EV%g_ix+2), &
                  0.75_dl*(y(EV%r_ix)-4.0_dl*convert_delta),phi,psi,y(2)/EV%q,sigma,adotoa
          else  
             dgs = grhog/a**2*y(EV%g_ix+2)+grhornomass/a**2*y(EV%r_ix+2)+grhog_dark/a**2*y(EV%gdark_ix+2)
             psi = phi - dgs/EV%q2       
             write (35,'(19E18.9)') tau,y(1),y(3)-3.0_dl*convert_delta,EV%q*y(4)+convert_theta,&
                  y(5)-3.0_dl*convert_delta,EV%q*y(6)+convert_theta,y(7)-3.0_dl*convert_delta, &
                  0.75_dl*(y(EV%gdark_ix)-4.0_dl*convert_delta),0.75_dl*EV%q*y(EV%gdark_ix+1)+convert_theta,y(EV%gdark_ix+2), &
